%computes a coherence (and power density)
%% Carl Wunsch, 1987
%of data yy(t) , zz(t), t=1,N, where N is a power of 2,
% N is computed from the size of yhat
%using fft.
% M is window width--Daniell. delt must also be specified.
%%function [spectry,spectrz, coheramp,coherpha,freq]=coher(yy,zz,delt,M,'title1','title2',plot);
%
%Phase convention: the phase angle is positive when the 2nd time series
%leads.
%
...
% Modified to include plot flag. [Cherian, 08-06-10]

function [spectry_out,spectrz_out, coheramp_out,coherpha_out,freq]=coher(yy,zz,delt,M,title1,title2,plot);
yy1=yy(:);zz1=zz(:);
